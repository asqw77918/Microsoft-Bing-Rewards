...
> [!NOTE]
> `ACCOUNT_N_PASSWORD` 是可选的。需要密码登录时设置它；仅使用无密码 / 验证器登录的账号可留空。
>
...
`ACCOUNT_N_PASSWORD` 是可选的；仅使用无密码 / 验证器登录的账号可省略。
...
| `contintueOnBotWarning`     | boolean | `false`      | 遇到 Microsoft 机器人评分警告时继续运行（不推荐）  | `CONFIG_CONTINTUE_ON_BOT_WARNING`     |
