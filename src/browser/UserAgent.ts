    ...
    async getChromeVersion(isMobile: boolean): Promise<string> {
        try {
            ...
            this.bot.logger.warn(
                ...
                `版本查询不可用（${error instanceof Error ? error.message : String(error)}）；正在使用内置回退版本 ${fallback}`
            )
            return fallback
        }
    }
    ...
    async getEdgeVersions(isMobile: boolean): Promise<EdgeVersions> {
        try {
            ...
            this.bot.logger.warn(
                ...
                `版本查询不可用（${error instanceof Error ? error.message : String(error)}）；正在使用内置回退版本 Android ${fallback.android}、Desktop ${fallback.windows}`
            )
