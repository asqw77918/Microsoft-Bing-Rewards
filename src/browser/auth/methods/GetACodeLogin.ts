...
export class CodeLogin {
    ...
    private async fillCode(page: Page, code: string): Promise<boolean> {
        try {
            ...
            if (visibleInput) {
                ...
                this.bot.logger.info(this.bot.isMobile, 'LOGIN-CODE', '已填写验证码输入框')
                return true
            }
    ...
    private async fillEmail(page: Page, email: string): Promise<boolean> {
        try {
            ...
            this.bot.logger.warn(this.bot.isMobile, 'LOGIN-CODE', '未找到邮箱验证输入框')
            return false
        } catch (error) {
            this.bot.logger.warn(
                this.bot.isMobile,
    ...
    async handle(page: Page): Promise<void> {
        try {
            this.bot.logger.info(this.bot.isMobile, 'LOGIN-CODE', '已请求验证码登录身份验证')
            ...
            if (emailMessage) {
                ...
                this.bot.logger.warn(this.bot.isMobile, 'LOGIN-CODE', '无法获取邮箱验证码目标')
            }
            ...
            if (emailProofInput) {
                ...
                this.bot.logger.debug(this.bot.isMobile, 'LOGIN-CODE', `正在使用 ${selectorVariant} 邮箱验证输入选择器`)
                const maskedHint = emailMessage?.match(/[A-Za-z0-9._*+-]+@[A-Za-z0-9.*-]+\.[A-Za-z]{2,}/)?.[0]
                this.bot.logger.info(this.bot.isMobile, 'LOGIN-CODE', '检测到邮箱验证挑战，正在等待邮箱地址')
                ...
                for (let attempt = 1; attempt <= this.maxManualAttempts; attempt++) {
                    ...
                    if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
                        this.bot.logger.warn(
                            this.bot.isMobile,
                            'LOGIN-CODE',
                            `邮箱无效或缺失（第 ${attempt}/${this.maxManualAttempts} 次尝试）| 输入长度=${email?.length}`
                        )
                    ...
                    if (!filled) {
                        this.bot.logger.error(
                            this.bot.isMobile,
                            'LOGIN-CODE',
                            `无法填写邮箱输入框（第 ${attempt}/${this.maxManualAttempts} 次尝试）`
                        )
                    ...
                    if (emailError) {
                        this.bot.logger.warn(
                            this.bot.isMobile,
                            'LOGIN-CODE',
                            `邮箱被拒绝: ${emailError}（第 ${attempt}/${this.maxManualAttempts} 次尝试）`
                        )
                    ...
                    this.bot.logger.info(this.bot.isMobile, 'LOGIN-CODE', '邮箱已接受，验证码已发送')
                    emailVerified = true
                    break
                }
            ...
            for (let attempt = 1; attempt <= this.maxManualAttempts; attempt++) {
                ...
                this.bot.logger.info(this.bot.isMobile, 'LOGIN-CODE', '验证码身份验证成功完成')
                return
            }
