...
export class PasswordlessLogin {
    ...
    private async waitForApproval(page: Page): Promise<Exclude<ApprovalState, 'PENDING'> | 'TIMED_OUT'> {
        this.bot.logger.info(
            this.bot.isMobile,
            'LOGIN-PASSWORDLESS',
            `正在等待批准...（${this.approvalTimeoutSeconds} 秒后超时）`
        )
        ...
        for (let elapsed = 1; elapsed <= this.approvalTimeoutSeconds; elapsed++) {
            ...
            if (elapsed % 5 === 0) {
                this.bot.logger.debug(
                    this.bot.isMobile,
                    'LOGIN-PASSWORDLESS',
                    `仍在等待...（已等待 ${elapsed}/${this.approvalTimeoutSeconds} 秒）`
                )
            }
        ...
        this.bot.logger.warn(
            this.bot.isMobile,
            'LOGIN-PASSWORDLESS',
            `等待批准超时（${this.approvalTimeoutSeconds} 秒）！`
        )
        return 'TIMED_OUT'
    }
    ...
    async handle(page: Page): Promise<void> {
        try {
            this.bot.logger.info(this.bot.isMobile, 'LOGIN-PASSWORDLESS', '已请求无密码身份验证')
            ...
            for (let retry = 0; retry <= this.maxRequestRetries; retry++) {
                ...
                if (state === 'RETRY_AVAILABLE' && retry < this.maxRequestRetries) {
                    this.bot.logger.warn(
                        this.bot.isMobile,
                        'LOGIN-PASSWORDLESS',
                        `批准请求已过期；正在发送重试 ${retry + 1}/${this.maxRequestRetries}`
                    )
