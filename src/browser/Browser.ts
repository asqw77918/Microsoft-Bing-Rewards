    ...
    async createBrowser(account: Account): Promise<BrowserCreationResult> {
        ...
        try {
            ...
            this.bot.logger.info(
                this.bot.isMobile,
                'BROWSER',
                `正在启动内置补丁版 Chromium（Edge UA）| headless=${headless} | platform=${process.platform} | proxy=${hasProxy ? 'yes' : 'no'} | tls=${ignoreCertificateErrors ? 'verification-disabled' : 'verified'} | sandbox=${sandboxDisabled ? 'disabled-root' : 'enabled'}`
            )
        ...
        try {
            ...
            if (savedFingerprint && !reuseFingerprint) {
                this.bot.logger.info(
                    this.bot.isMobile,
                    'BROWSER-FINGERPRINT',
                    `已保存指纹的语言区域与 ${this.bot.accountLocale.locale} 不匹配；正在生成替代指纹`
                )
            }
