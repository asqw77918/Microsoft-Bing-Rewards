    ...
    async tryDismissAllMessages(page: Page): Promise<void> {
        try {
            ...
            if (overlay) {
                ...
                if (rejected) {
                    this.bot.logger.debug(this.bot.isMobile, 'DISMISS-ALL-MESSAGES', '已关闭: Bing Overlay 拒绝')
                } else {
                    const accepted = await this.ghostClick(page, '#bnp_btn_accept')
                    if (accepted) {
                        this.bot.logger.debug(this.bot.isMobile, 'DISMISS-ALL-MESSAGES', '已关闭: Bing Overlay 接受')
                    }
                }
    ...
    async closeTabs(page: Page, config = { minTabs: 1, maxTabs: 1 }): Promise<Page> {
        try {
            ...
            this.bot.logger.debug(
                this.bot.isMobile,
                'SEARCH-CLOSE-TABS',
                `发现 ${tabs.length} 个打开的标签页（最小: ${config.minTabs}，最大: ${config.maxTabs}）`
            )

            if (config.minTabs < 1 || config.maxTabs < config.minTabs) {
                this.bot.logger.warn(this.bot.isMobile, 'SEARCH-CLOSE-TABS', '无效配置，使用默认值')
            ...
            if (tabs.length > config.maxTabs) {
                ...
                this.bot.logger.debug(
                    this.bot.isMobile,
                    'SEARCH-CLOSE-TABS',
                    `已关闭 ${closedCount}/${tabsToClose.length} 个多余标签页以达到最大 ${config.maxTabs}`
                )
                ...
                this.bot.logger.debug(
                    this.bot.isMobile,
                    'SEARCH-CLOSE-TABS',
                    `正在打开 ${tabsNeeded} 个标签页以达到最小 ${config.minTabs}`
                )
    ...
    async ghostClick(page: Page, selector: string, options?: ClickOptions): Promise<boolean> {
        try {
            this.bot.logger.debug(
                this.bot.isMobile,
                'GHOST-CLICK',
                `正在尝试点击选择器: ${selector}，选项: ${JSON.stringify(options)}`
            )
            ...
            this.bot.logger.warn(
                this.bot.isMobile,
                'GHOST-CLICK',
                `点击失败 ${selector}: ${error instanceof Error ? error.message : String(error)}`
            )
            return false
        }
    }
