...
export class BonusTracker implements SearchTracker {
    ...
    async prepare(): Promise<boolean> {
        if (this.maxSearches <= 0) {
            this.bot.logger.info(this.isMobile, this.context, 'maxBonusSearches 为 0，跳过奖励搜索')
            return false
        }
        ...
        try {
            ...
            this.bot.logger.warn(
                this.isMobile,
                this.context,
                `无法获取仪表盘，跳过奖励搜索 | ${error instanceof Error ? error.message : String(error)}`
            )
            return false
        }
        ...
        this.bot.logger.info(
            this.isMobile,
            this.context,
            `找到搜索奖励 "${offer.title}" | offerId=${this.offerId} | progress=${this.current}/${this.max} | maxSearches=${this.maxSearches}`
        )
        return true
    }
    ...
    async measure(): Promise<number> {
        ...
        if (!cur) {
            this.bot.logger.warn(this.isMobile, this.context, `活动 ${this.offerId} 已不存在，正在停止`)
            this.offerLost = true
            return 0
        }
