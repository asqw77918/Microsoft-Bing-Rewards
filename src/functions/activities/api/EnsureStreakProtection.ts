...
export class EnsureStreakProtection extends BaseActivity {
    public async ensureStreakProtection() {
        ...
        if (!resolved) {
            this.bot.logger.warn(
                ...
                `跳过：未在 bundle 中发现连续天数保护操作 id（已查找 [${STREAK_PROTECTION_ACTION_NAMES.join(', ')}] 及任意 "*streak*protect*" 键）`
            )
            return
        }
        ...
        if (before?.isProtectionOn) {
            this.bot.logger.info(
                this.bot.isMobile,
                'ENABLE-STREAK-PROTECTION',
                `已启用 | remainingDays=${before.remainingDays ?? 'null'}`,
                'green'
            )
            return
        }
        ...
        this.bot.logger.info(
            this.bot.isMobile,
            'ENABLE-STREAK-PROTECTION',
            `开始执行 EnsureStreakProtection | action=${resolved.name} | before=${beforeDesc}`
        )
        ...
        try {
            ...
            if (after?.isProtectionOn) {
                this.bot.logger.info(
                    this.bot.isMobile,
                    'ENABLE-STREAK-PROTECTION',
                    `已完成 | streakProtectionEnabled=true | remainingDays=${after.remainingDays ?? 'null'} | status=${status}`,
                    'green'
                )
            } else if (after === null) {
                this.bot.logger.warn(
    ...
    private async readStreakProtection() {
        try {
            ...
            if (!html) {
                this.bot.logger.warn(this.bot.isMobile, 'ENABLE-STREAK-PROTECTION', '验证获取失败')
                return null
            }
