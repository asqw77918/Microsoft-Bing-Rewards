...
export class EdgeBrowsing extends BaseActivity {
    public async run(data: DashboardData, signal?: AbortSignal): Promise<void> {
        ...
        try {
            ...
            if (complete) {
                this.bot.logger.info(this.bot.isMobile, LOG_TAG, 'Edge 上的浏览 Streak 已完成')
                return
            }
            ...
            this.bot.logger.info(
                this.bot.isMobile,
                LOG_TAG,
                `已开始后台 Edge 浏览活动 | offerId=${settings.offerId} | type=${settings.activityType}` +
            ...
            for (let reportNumber = 1; reportNumber <= reportCount; reportNumber++) {
                ...
                try {
                    ...
                    this.bot.logger.warn(
                        this.bot.isMobile,
                        LOG_TAG,
                        `Edge 浏览上报失败 | report=${reportNumber}/${reportCount}` +
                ...
                const message =
                    `已提交 Edge 浏览上报 | report=${reportNumber}/${reportCount} | status=${result.status}` +
            ...
            const summary =
                `后台 Edge 浏览活动已完成 | reports=${reportsProcessed}` +
    ...
    private async activate(data: DashboardData, signal?: AbortSignal): Promise<ActivationResult> {
        ...
        if (!offer) {
            this.bot.logger.debug(
                this.bot.isMobile,
                LOG_TAG,
                '当前、桌面或缓存的移动端 Rewards 快照中的 streak 模型或通用活动中均未找到 Edge 浏览激活元数据'
            )
            return 'absent'
        }
        ...
        if (offer.isCompleted) {
            this.bot.logger.info(
                this.bot.isMobile,
                LOG_TAG,
                `Edge 浏览激活活动已完成 | offerId=${offer.offerId}`,
                'green'
            )
        ...
        this.bot.logger.info(
            this.bot.isMobile,
            LOG_TAG,
            `正在激活 Edge 上的浏览 Streak | offerId=${offer.offerId} | activationSource=${offer.activationSource} | activityType=${metadata.activityType} | activityTypeSource=${metadata.activityTypeSource} | promotional=${metadata.isPromotional} | geo=${this.bot.userData.geoLocale}`
        )
        ...
        try {
            ...
            if (acknowledged || confirmed) {
                this.bot.logger.info(
                    this.bot.isMobile,
                    LOG_TAG,
                    `Edge 上的浏览 Streak 已激活 | offerId=${offer.offerId} | acknowledged=${acknowledged} | confirmed=${confirmed}`,
                    'green'
                )
                return 'activated'
            }
            ...
            this.bot.logger.warn(
                this.bot.isMobile,
                LOG_TAG,
                `激活未获确认 | offerId=${offer.offerId} | status=${status}`
            )
            return 'failed'
        } catch (error) {
            this.bot.logger.error(
        ...
        for (const { source, snapshot } of snapshots) {
            ...
            if (source !== 'current') {
                this.bot.logger.debug(
                    this.bot.isMobile,
                    LOG_TAG,
                    `当前快照中缺少激活元数据；正在使用缓存的 ${source} streak 快照 | offerId=${streak.activationOfferId}`
                )
            } else {
                this.bot.logger.debug(
                    this.bot.isMobile,
                    LOG_TAG,
                    `正在使用 streak 模型中的 Edge 浏览激活元数据 | offerId=${streak.activationOfferId}`
                )
            }
        ...
        for (const { source, snapshot } of snapshots) {
            ...
            if (source !== 'current') {
                ...
                this.bot.logger.debug(
                    this.bot.isMobile,
                    LOG_TAG,
                    `使用通用活动中的 Edge 浏览激活元数据 | offerId=${offer.offerId}`
                )
            }
        ...
        try {
            ...
            if (!promotion) {
                this.bot.logger.debug(
                    this.bot.isMobile,
                    LOG_TAG,
                    `无法验证 Edge 浏览完成情况：活动不存在 | report=${reportNumber}/${reportCount}`
                )
                return false
            }
    ...
    private logServerComplete(
        ...
        this.bot.logger.info(
            this.bot.isMobile,
            LOG_TAG,
            `Microsoft 报告 Edge 浏览活动已完成 | report=${reportNumber}/${reportCount}` +
                ` | accepted=${acceptedReports} | duplicates=${duplicateReports} | failed=${failedReports}`,
            'green'
        )
    }
