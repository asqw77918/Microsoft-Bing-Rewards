...
export class PromotionActivityRunner extends BaseActivity {
        ...
        for (const [index, promotion] of promotions.entries()) {
            try {
                ...
                this.bot.logger.error(
                    this.bot.isMobile,
                    'ACTIVITY',
                    `处理活动 "${promotion.title}" 时出错 | message=${
    ...
    private async runPromotion(promotion: BasePromotion): Promise<void> {
        ...
        this.bot.logger.debug(
            this.bot.isMobile,
            'ACTIVITY',
            `正在处理活动 | title="${promotion.title}" | offerId=${offerId} | type=${type}`
        )
        ...
        if (isSearchOnBing) {
            this.bot.logger.info(
                this.bot.isMobile,
                'ACTIVITY',
                `找到活动类型 "SearchOnBing" | title="${promotion.title}" | offerId=${offerId}`
            )
        ...
        this.bot.logger.info(
            this.bot.isMobile,
            'ACTIVITY',
            `找到活动类型 "UrlReward" | title="${promotion.title}" | offerId=${offerId}`
        )
