...
export class PunchCards extends BaseActivity {
    public async runMobile(data: DashboardData): Promise<void> {
        try {
            ...
            this.bot.logger.error(
                'main',
                'PUNCHCARD',
                `移动端 punchcards 失败 | ${error instanceof Error ? error.message : String(error)}`
            )
        }
    }
    ...
    public async runDesktop(): Promise<void> {
        ...
        try {
            ...
            this.bot.logger.warn(
                'main',
                'PUNCHCARD',
                `桌面端 punchcard 数据不可用 | ${error instanceof Error ? error.message : String(error)}`
            )
            return
        }
        ...
        try {
            ...
            this.bot.logger.error(
                'main',
                'PUNCHCARD',
                `桌面端 punchcards 失败 | ${error instanceof Error ? error.message : String(error)}`
            )
        }
    }
    ...
    private async run(data: DashboardData): Promise<void> {
        ...
        this.bot.logger.info(
            this.bot.isMobile,
            'PUNCHCARD',
            `找到 ${pending.length} 个未完成任务 | apiMatched=${pending.filter(parent => apiById.has(parent.offerId)).length}`
        )
    ...
    private async getParentQuests(): Promise<ParentQuest[] | null> {
        try {
            ...
            if (!earn) {
                this.bot.logger.warn(this.bot.isMobile, 'PUNCHCARD', '/earn 不可用 - 无法列出任务')
                return null
            }
    ...
    private async solvePunchCard(parent: ParentQuest, apiCard: PunchCard | undefined): Promise<void> {
        ...
        for (const child of ordered) {
            ...
            if (this.isSearchQuotaChild(child.offerId, apiChild)) {
                remaining += 1
                this.bot.logger.info(this.bot.isMobile, 'PUNCHCARD', `跳过 ${child.offerId}：多日搜索任务`)
                continue
            }
            ...
            if (this.isClaimChild(child.offerId, apiChild)) {
                if (!this.bot.config.autoClaimPunchcardRewards) {
                    ...
                    this.bot.logger.info(
                        this.bot.isMobile,
                        'PUNCHCARD',
                        `"${title}" 的奖励已可手动兑换 | offerId=${child.offerId}`
                    )
                    continue
                }
        ...
        this.bot.logger.info(
            this.bot.isMobile,
            'PUNCHCARD',
            `任务 "${title}" ${remaining === 0 ? '已完成' : '进行中'} | reported=${reported}` +
    ...
    private async getQuestChildren(parentId: string, title: string): Promise<QuestChild[] | null> {
        try {
            ...
            if (!children.length) {
                this.bot.logger.info(this.bot.isMobile, 'PUNCHCARD', `"${title}" 没有可操作的子任务`)
                return null
            }
            return children
        } catch (error) {
    ...
    private async reportQuestChild(child: QuestChild, parentId: string): Promise<void> {
        ...
        try {
            ...
            this.bot.logger.info(
                this.bot.isMobile,
                'PUNCHCARD',
                `子任务已上报 | offerId=${child.offerId} | status=${status} | acknowledged=${acknowledged}` +
            ...
            this.bot.logger.error(
                this.bot.isMobile,
                'PUNCHCARD',
                `上报子任务时出错 | offerId=${child.offerId} | message=${
                    error instanceof Error ? error.message : String(error)
                }`
            )
        }
