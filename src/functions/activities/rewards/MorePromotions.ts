...
export class MorePromotions extends BaseActivity {
    public async run(data: DashboardData): Promise<void> {
        ...
        if (!pending.length) {
            this.bot.logger.info(this.bot.isMobile, 'MORE-PROMOTIONS', '所有 "More Promotions" 项目均已完成')
            return
        }
        ...
        this.bot.logger.info(
            this.bot.isMobile,
            'MORE-PROMOTIONS',
            `开始处理 "More Promotions" 项目 | remaining=${pending.length}`
        )
        await new PromotionActivityRunner(this.bot).run(pending)
        this.bot.logger.info(this.bot.isMobile, 'MORE-PROMOTIONS', '"More Promotions" 项目处理完成')
    }
