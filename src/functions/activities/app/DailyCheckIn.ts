...
export class DailyCheckIn extends BaseActivity {
    ...
    public async doDailyCheckIn() {
        if (!this.bot.accessToken) {
            this.bot.logger.warn(this.bot.isMobile, 'DAILY-CHECK-IN', '跳过：应用访问令牌不可用，此活动需要该令牌！')
            return
        }
        ...
        this.bot.logger.info(
            this.bot.isMobile,
            'DAILY-CHECK-IN',
            `开始执行每日签到 | geo=${this.bot.userData.geoLocale} | currentBalance=${this.oldBalance}`
        )
        ...
        try {
            ...
            this.bot.logger.debug(
                this.bot.isMobile,
                'DAILY-CHECK-IN',
                `收到每日签到响应 | status=${response?.status ?? 'unknown'}`
            )
            ...
            if (this.gainedPoints > 0) {
                ...
                this.bot.logger.info(
                    this.bot.isMobile,
                    'DAILY-CHECK-IN',
                    `Daily Check-In 已完成 | type=103 | pointsGained=${this.gainedPoints} | currentBalance=${newBalance}`,
                    'green'
                )
            } else {
                this.bot.logger.warn(
                    this.bot.isMobile,
                    'DAILY-CHECK-IN',
                    `每日签到已完成但未获得积分 | type=103 | pointsGained=0 | currentBalance=${newBalance}`
                )
            }
        } catch (error) {
            this.bot.logger.error(
                this.bot.isMobile,
                'DAILY-CHECK-IN',
                `每日签到执行出错 | message=${error instanceof Error ? error.message : String(error)}`
            )
        }
    }
    ...
    private async submitDaily() {
        try {
            ...
            this.bot.logger.debug(
                this.bot.isMobile,
                'DAILY-CHECK-IN',
                `正在准备每日签到请求数据 | type=${jsonData.type} | id=${jsonData.id} | amount=${jsonData.amount} | country=${jsonData.country}`
            )
