...
export class AppReward extends BaseActivity {
    ...
    public async doAppReward(promotion: Promotion) {
        if (!this.bot.accessToken) {
            this.bot.logger.warn(this.bot.isMobile, 'APP-REWARD', '跳过：应用访问令牌不可用，此活动需要该令牌！')
            return
        }
        ...
        this.bot.logger.info(
            this.bot.isMobile,
            'APP-REWARD',
            `开始执行 AppReward | offerId=${offerId} | country=${this.bot.userData.geoLocale} | currentBalance=${this.oldBalance}`
        )
        ...
        try {
            ...
            this.bot.logger.debug(
                this.bot.isMobile,
                'APP-REWARD',
                `活动请求数据已准备 | offerId=${offerId} | id=${jsonData.id} | amount=${jsonData.amount} | type=${jsonData.type} | country=${jsonData.country}`
            )
            ...
            this.bot.logger.debug(
                this.bot.isMobile,
                'APP-REWARD',
                `收到活动响应 | offerId=${offerId} | status=${response.status}`
            )
            ...
            if (this.gainedPoints > 0) {
                ...
                this.bot.logger.info(
                    this.bot.isMobile,
                    'APP-REWARD',
                    `AppReward 已完成 | offerId=${offerId} | pointsGained=${this.gainedPoints} | currentBalance=${newBalance}`,
                    'green'
                )
            } else {
                this.bot.logger.warn(
                    this.bot.isMobile,
                    'APP-REWARD',
                    `AppReward 已完成但未获得积分 | offerId=${offerId} | pointsGained=0 | currentBalance=${newBalance}`
                )
            }
            ...
            this.bot.logger.info(
                this.bot.isMobile,
                'APP-REWARD',
                `AppReward 结束 | offerId=${offerId} | currentBalance=${this.bot.userData.currentPoints}`
            )
        } catch (error) {
